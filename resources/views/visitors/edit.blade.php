...
@section('title', 'Edit Visitor - ACTS Church CMS')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">{{ old('address', $visitor->address) }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="invited_by" value="{{ old('invited_by', $visitor->invited_by) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500 focus:border-gold-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="follow_up_status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">
                        ...
                        <option value="follow_up" {{ old('follow_up_status', $visitor->follow_up_status) === 'follow_up' ? 'selected' : '' }}>Follow Up</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="follow_up_notes" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">{{ old('follow_up_notes', $visitor->follow_up_notes) }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Prayer Request</label>
                    <textarea name="prayer_request" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">{{ old('prayer_request', $visitor->prayer_request) }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                    <textarea name="notes" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">{{ old('notes', $visitor->notes) }}</textarea>
                </div>
            </div>
        </div>
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('visitors.show', $visitor) }}" class="px-5 py-2.5 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancel</a>
