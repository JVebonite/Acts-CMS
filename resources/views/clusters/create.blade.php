...
@section('title', 'Create Cluster - ACTS Church CMS')
@section('page-title', 'Create Cluster')
@section('page-description', 'Set up a new cluster group')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="leader_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">
                        ...
                        <option value="{{ $m->id }}" {{ old('leader_id') == $m->id ? 'selected' : '' }}>{{ $m->full_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gold-500">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('clusters.index') }}" class="px-5 py-2.5 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancel</a>
